...
with open(Path(workspace.name)/'chrome.log','w') as log:
 ...
 try:
  ...
  time.sleep(.1)
  check(ev(f,"Array.from(document.querySelectorAll('#context-list strong')).map(x=>x.textContent).join('|')==='Jordan Lee|Jordan Lee|Casey Smith|Unknown speaker|Unknown speaker|Unknown speaker'"),'grouped messages keep sender without leaking across dividers or mentions')
